window.onload = function() {
	var check=true;
		...
		clearInterval(timer);
		//
		var moveul=document.getElementById('move');
		var morebth=document.getElementById('more');
		var allbox=document.getElementById('allbox');
		var alla =move.getElementsByTagName("a");
		alla[0].style.backgroundColor='red';
		alla[0].style.color='white';
		for(var i=0;i<alla.length;i++){
			...
			alla[i].onclick=function(){
				...
				alla[this.index].style.backgroundColor='red';
				alla[this.index].style.color='white';
			}
		}
		morebth.onclick=function(){
			if(check){
				allbox.style.left='-25%';
		move.style.width='25%';
		this.src='img/更多2.png';
		allbox.style.boxShadow='0 0 10px silver'; 
			}else{
				allbox.style.left=0;
				move.style.width=0;
				this.src='img/更多 (1).png';
			   	allbox.style.boxShadow='none'; 
			}
		check=!check
		}
